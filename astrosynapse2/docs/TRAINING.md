...
Astro3 may reject an obviously inferior candidate at predeclared geometric looks after at least the configured minimum number of pairs. Each one-sided Hoeffding upper bound uses a Bonferroni share of the configured family-wise error budget, so constant observations cannot create a false zero-width interval. After the initial promotion target, any candidate scoring above 50% whose paired interval adjusted for all planned looks still overlaps 50% continues in resumable 2,000-pair blocks. It stops when the interval is wholly above 50%, the score is no longer above 50%, or 100,000 pairs are complete. Manual GUI/API arenas never promote models.

Truncations remain visible rather than being silently treated as ordinary draws. For automatic promotion, every truncated game is conservatively rescored as a candidate loss; promotion is allowed only when the adjusted paired confidence interval still clears the normal threshold and every other gate passes. A truncated result that cannot clear that conservative test remains retryable.
