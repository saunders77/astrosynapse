...
- `GET /presets` — validated `astro5_directional`, `astro5_mature`, `astro5_search`, earlier-generation compatibility, and `quick` configurations. Directional refinement is intended for Branch Lab because it requires a source checkpoint with retained verified promotions.
...
The selected checkpoint plays both seats using its greedy mean-head deployment policy. An acquire choice rates the selected purchase or free-acquire card against every other card legal in that decision. `No Card` receives a separate head-to-head comparison only against the card actually acquired on a one-acquire turn; on a zero-acquire turn it beats every card affordable with the trade remaining when the turn ended. A scrap choice rates the selected hand/discard card or `No Discard` against every other option legal in that decision, with hand and discard evidence combined into one score per card. The analyzer groups events by player turn and rejects the complete turn if more than one card was acquired or scrapped; standard and free acquisitions both count, and hand/discard plus in-play scrap-for-ability actions all count toward the scrap filter. Acquire Elo uses the original multinomial/Plackett-Luce update and is normalized so Explorer is `2.0`; Scrap Elo uses the original pairwise update.
