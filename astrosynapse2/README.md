...
2. Select **Promotion-direction refinement** for an already-successful model with retained promotion history. It builds a consensus from the five latest verified promotion deltas and guides only weight coordinates whose historical signs agree. Its full gate checks every 2,000 pairs from the first 2,000 through 98,000: a familywise 99% bound can reject an obvious loser or promote an obvious winner. Anything still unresolved reaches the 100,000-pair final boundary, which uses the reserved 4% error budget (96% standalone confidence) so the complete sequential procedure remains 95% familywise. Branch Lab exposes the history, consensus, strength, and evaluation limits before creation. **Mature champion refinement** is the direction-agnostic alternative; the other recipes test balanced search, heavier search, entropy recovery, value emphasis, faster exploitation, broader belief search, low-LR long memory, and higher exploration.
