...
The September improvement campaign has a dedicated [Astro6 progress view](http://127.0.0.1:3000/progressive)
with live training, independently verified champion succession, and pause/resume controls.
...
The existing Astro5 preset uses the following M4/16 GB configuration (see the [September assessment](docs/assessment-2026-09-09.md) for its measured plateau): 8 CPU actors, four work-stealing tasks per actor, a 192-wide three-block model, 5 policy heads, a 250,000-decision hot replay window in RAM, and up to 5 million older decisions in memory-mapped SSD shards. Both tiers retain at most 12 phase/family-balanced decisions per player-game, and 30% of each learner batch comes from the older tier when it is populated. Search independently samples each eligible main-phase decision with probability 0.25%, capped at one position per game, compares four actions with one shared-randomness rollout each, and bootstraps from the network value after a two-turn lookahead instead of simulating every branch to termination. The preset checkpoints and runs a 64-pair canary every 10,000 games, and starts a 2,000-pair promotion evaluation every 50,000 games. A confidence-safe acceptance test may promote an unambiguously strong candidate from 1,000 pairs; ambiguous candidates run all 2,000. After any promotion evaluation reaches its initial target, a score above 50% whose paired interval adjusted for all planned looks still overlaps 50% adds another 2,000 pairs to the same resumable job. This repeats until the interval is wholly above 50%, the score is no longer above 50%, or 100,000 total pairs are complete. Rejected candidates remain quarantined from deployment but continue learning, which lets them cross temporary performance valleys. Optimization health is reconsidered every 500 games, while strategic governor decisions change only when new canary evidence arrives. Astro4 remains available as the previous legal-set control.
...
- **Acquire students** — train one small decision tree from any playable checkpoint, follow durable training progress, inspect test agreement and the complete English tree, and download sampled positions. Both Play modes can show its next-acquisition forecast using an explicit full-turn trade input. The default tree cap is 11 total nodes. See [sampling, trade, and fitting details](docs/ACQUIRE_STUDENTS.md).
