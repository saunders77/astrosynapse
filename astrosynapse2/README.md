...
- **Branch Lab** — choose a compatible generation-4/5 checkpoint from any run, select any combination of optimization/search variants, and optionally run the queue automatically. Source checkpoints are pinned and every branch receives its own seed stream; mature refinement deliberately starts with a fresh optimizer/replay while other variants import all compatible lineage artifacts. Branches do not continue from the preceding branch's champion. Per-branch budgets can be expressed in minutes, training games, or completed full evaluations. The Branch runner controls always pause, resume, or stop the actually active trainer even when a queued branch is selected for inspection.
...
2. Select **Mature champion refinement** for an already-successful model, or check any of the comparative variants. Mature refinement performs clean local trials around the imported champion, restores the champion after a failed full gate, and can extend a positive borderline gate from 2,000 up to 12,000 pairs by default. Repeated looks use adjusted confidence. Other built-in recipes test balanced search, heavier search, entropy recovery, value emphasis, faster exploitation, broader belief search, low-LR long memory, and higher exploration.
