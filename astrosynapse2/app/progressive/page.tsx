...
type Gate = { games: number; score: number; lower: number; confidence: number; pairs: number; passed: boolean; stage: number; opponent: string };
type Progress = {
  available: boolean; name: string; phase: string; running: boolean; stale: boolean; stop_requested: boolean;
...
export default function ProgressiveTraining() {
  ...
  const phase = data?.stop_requested ? "Saving and pausing" : data?.phase?.replaceAll("_", " ");
  return <main className="progressive-page">
    ...
    <div className="progressive-heading"><div><p className="progressive-kicker">ASTRO6 / SUCCESSIVE CHAMPIONS</p><h1>Progressive champion training</h1><p>Learn, verify an improvement, then train against the new champion.</p></div>
      {data?.available && <button disabled={busy || data.stop_requested} onClick={() => control(data.running ? "pause" : "resume")}>{data.running ? "Pause & save" : "Resume training"}</button>}
    </div>
    ...
    {!data?.available ? <section className="progressive-panel">Waiting for the progressive training process…</section> : <>
      ...
      <section className="progressive-panel"><div className="progressive-panel-heading"><h2>Checkpoint progress</h2><span>Blue: screening · Green: verified promotion</span></div>
        <svg viewBox="0 0 880 235" role="img" aria-label="Checkpoint win rates against the incumbent champion over training games">
          ...
          {screens.map((s, i) => <circle key={i} cx={x(s.games)} cy={y(s.score)} r="4" fill="#70a9ff"><title>{number(s.games)} games: {pct(s.score)} vs {s.opponent}</title></circle>)}
      ...
      <section className="progressive-panel"><h2>Independent verification results</h2><div className="progressive-table"><table><thead><tr><th>Training games</th><th>Opponent</th><th>Evaluation games</th><th>Score</th><th>Lower bound</th><th>Confidence</th><th>Result</th></tr></thead><tbody>
        {data.gates.slice(-20).reverse().map((gate, i) => <tr key={i}><td>{number(gate.games)}</td><td>{gate.opponent}</td><td>{number(gate.pairs * 2)}</td><td>{pct(gate.score)}</td><td>{pct(gate.lower)}</td><td>{pct(gate.confidence)}</td><td style={{color: gate.passed ? "var(--mint)" : "var(--muted)"}}>{gate.passed ? "Promoted" : "Not proven stronger"}</td></tr>)}
        {!data.gates.length && <tr><td colSpan={7}>No independent promotion test has finished yet.</td></tr>}
      </tbody></table></div></section>
      <p className="progressive-footer">You can close this page while training continues. Pause & save finishes the current work safely. This campaign uses historical rules for comparison with existing champions.</p>
    </>}
  </main>;
}
